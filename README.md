...
- 📏 **分辨率控制**: 支持 `--1k` `--2k` `--4k`，并以 Vertex 官方 `imageConfig` 参数传递
...
- **aspect_ratio**: 生成图像的默认宽高比，留空则由模型自动决定
  ...
  - 会通过 Vertex 官方 `generationConfig.imageConfig.aspectRatio` 字段传递
  - 也可在指令中用标志覆盖（见下方指令说明）

- **default_resolution**: 生成图像的默认分辨率（像素），设为 `0` 则由模型自动决定
  - 可选值：`0`（自动）、`1024`（1K）、`2048`（2K）、`4096`（4K）
  - 会映射为 Vertex 官方 `generationConfig.imageConfig.imageSize` 的 `1K` / `2K` / `4K`
  - 也可在指令中用 `--1k` / `--2k` / `--4k` 标志覆盖
  - 实际输出仍受当前模型能力限制；若模型不支持指定尺寸，服务端可能回退或拒绝请求
